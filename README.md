...
The `grep.dfy` utility receives a pattern and a file, reads the file and prints all the lines from the file that have the pattern in it, the pattern is also highlighted with a different colour (red). In cases where the pattern is "atat" and there is a section with "atatat" the text will appear with only the first "atat" with the red colour "<span style="color:red;">atat</span>at", like this because the method searches for a pattern and doesn't use any part of that pattern to match another pattern. In a case where the section was for example "atatatat" the output would be "<span style="color:red;">atatatat</span>" because the second "atat" doesn't use any part of the first match. For the pattern it is not needed to have "" but if the pattern has spaces it is.
